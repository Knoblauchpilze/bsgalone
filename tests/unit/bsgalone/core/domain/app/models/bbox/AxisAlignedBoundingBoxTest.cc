...
TEST(Unit_Bsgalone_Core_Domain_App_Models_Bbox_AxisAlignedBoundingBox, PositionMatchesTheCenter)
{
  Eigen::Vector3f center(1.2f, -8.32f, 41.27f);
  Eigen::Vector3f dims(5.6f, 8.32f, 2.7f);
  AxisAlignedBoundingBox box(center, dims);

  EXPECT_EQ(center, box.position());
}

TEST(Unit_Bsgalone_Core_Domain_App_Models_Bbox_AxisAlignedBoundingBox, DimsAreAsGivenWhenConstructin)
{
  Eigen::Vector3f center(1.2f, -8.32f, 41.27f);
  Eigen::Vector3f dims(5.6f, 8.32f, 2.7f);
  AxisAlignedBoundingBox box(center, dims);
...
TEST(Unit_Bsgalone_Core_Domain_App_Models_Bbox_AxisAlignedBoundingBox, MoveToChangesCenter)
{
  Eigen::Vector3f aPlace(1.2f, -8.32f, 41.27f);
  Eigen::Vector3f dims(5.6f, 8.32f, 2.7f);
  AxisAlignedBoundingBox box(aPlace, dims);
...
TEST(Unit_Bsgalone_Core_Domain_App_Models_Bbox_AxisAlignedBoundingBox, TranslateChangesCenter)
{
  Eigen::Vector3f aPlace(1.2f, -8.32f, 41.27f);
  Eigen::Vector3f dims(5.6f, 8.32f, 2.7f);
  AxisAlignedBoundingBox box(aPlace, dims);
...
TEST_P(IsInsideTest, CorrectlyReturnsInsideStatus)
{
  ...
  AxisAlignedBoundingBox box(param.center, param.dims);

  EXPECT_EQ(param.isInside, box.isInside(param.position));
}

INSTANTIATE_TEST_SUITE_P(Unit_Bsgalone_Core_Domain_App_Models_Bbox_AxisAlignedBoundingBox,
                         IsInsideTest,
                         Values(
                           TestCaseIsInside{
