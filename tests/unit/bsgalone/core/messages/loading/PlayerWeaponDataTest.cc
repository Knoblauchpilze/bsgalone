
#include "PlayerWeaponData.hh"
#include "Common.hh"
#include "Comparison.hh"
#include "SerializationHelper.hh"
#include <gtest/gtest.h>

using namespace test;
using namespace ::testing;

namespace bsgalone::core {

TEST(Unit_Bsgalone_Core_Messages_PlayerWeaponData, EqualWhenDbIdIsEqual)
{
  PlayerWeaponData data1{.dbId       = Uuid{1234},
                         .weaponDbId = Uuid{41},
                         .minDamage  = 36.7f,
                         .range      = 123.456f};

  PlayerWeaponData data2{.dbId       = Uuid{1234},
                         .level      = 5,
                         .maxDamage  = 17.5f,
                         .reloadTime = chrono::TickDuration::fromInt(158)};

  EXPECT_TRUE(data1 == data2);
}

TEST(Unit_Bsgalone_Core_Messages_PlayerWeaponData, DifferentWhenDbIdIsDifferent)
{
  PlayerWeaponData data1{.dbId       = Uuid{1234},
                         .weaponDbId = Uuid{67},
                         .minDamage  = 5.0f,
                         .powerCost  = 100.0f};

  PlayerWeaponData data2 = data1;
  data2.dbId             = Uuid{5678};

  EXPECT_FALSE(data1 == data2);
}

TEST(Unit_Bsgalone_Core_Messages_PlayerWeaponData, Basic)
{
  PlayerWeaponData input{.dbId         = Uuid{1234},
                         .slotPosition = Eigen::Vector3f{1.2f, 4.3f, -5.7f},
                         .name         = "a weapon",
                         .level        = 4,
                         .minDamage    = 5.4321f,
                         .range        = 98765.1234f};

  PlayerWeaponData output{.dbId       = Uuid{14},
                          .maxDamage  = 12.987f,
                          .reloadTime = chrono::TickDuration(1234.456f)};

  EXPECT_TRUE(test::serializeAndDeserialize(input, output));

  assertPlayerWeaponDataAreEqual(output, input);
}

TEST(Unit_Bsgalone_Core_Messages_PlayerWeaponData, ErasesDestinationSlotPosition)
{
  PlayerWeaponData input{.dbId = Uuid{1234}, .level = 4, .minDamage = 5.4321f, .range = 98765.1234f};

  PlayerWeaponData output{.dbId         = Uuid{14},
                          .slotPosition = Eigen::Vector3f{1.2f, 4.3f, -5.7f},
                          .maxDamage    = 12.987f,
                          .reloadTime   = chrono::TickDuration(1234.657102f)};

  EXPECT_TRUE(test::serializeAndDeserialize(input, output));

  assertPlayerWeaponDataAreEqual(output, input);
}

TEST(Unit_Bsgalone_Core_Messages_PlayerWeaponData, OverridesDestinationSlotPosition)
{
  PlayerWeaponData input{.dbId         = Uuid{1234},
                         .slotPosition = Eigen::Vector3f{-7.8f, 97.54f, -478.52f},
                         .name         = "powerful weapon",
                         .level        = 4,
                         .minDamage    = 5.4321f,
                         .range        = 98765.1234f};

  PlayerWeaponData output{.dbId         = Uuid{14},
                          .slotPosition = Eigen::Vector3f{1.2f, 4.3f, -5.7f},
                          .name         = "another weapon",
                          .maxDamage    = 12.987f,
                          .reloadTime   = chrono::TickDuration::fromInt(1234)};

  EXPECT_TRUE(test::serializeAndDeserialize(input, output));

  assertPlayerWeaponDataAreEqual(output, input);
}

} // namespace bsgalone::core
