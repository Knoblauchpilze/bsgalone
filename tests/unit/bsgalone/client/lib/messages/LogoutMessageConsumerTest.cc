...
TEST(Unit_Bsgalone_Client_Messages_LogoutMessageConsumer, ThrowsWhenDataStoreIsNull)
{
  EXPECT_THROW(
    []() {
      LogoutMessageConsumer(nullptr,
                            std::make_shared<MockGame>(),
                            std::make_shared<TestUiEventQueue>());
    }(),
    std::invalid_argument);
}

TEST(Unit_Bsgalone_Client_Messages_LogoutMessageConsumer, ThrowsWhenGameIsNull)
{
  EXPECT_THROW(
    []() {
      LogoutMessageConsumer(std::make_shared<MockDataStore>(),
                            nullptr,
                            std::make_shared<TestUiEventQueue>());
    }(),
    std::invalid_argument);
...
TEST(Unit_Bsgalone_Client_Messages_LogoutMessageConsumer, ChecksLoginStatusFromStore)
{
  ...
  LogoutMessageConsumer consumer(mockStore,
                                 std::make_shared<MockGame>(),
                                 std::make_shared<TestUiEventQueue>());
     ...
     PublishesLogoutEventWhenPlayerIsNoLongerLoggedin)
{
  ...
  LogoutMessageConsumer consumer(mockStore, std::make_shared<MockGame>(), queue);
...
TEST(Unit_Bsgalone_Client_Messages_LogoutMessageConsumer, ResetsTheGameWhenLogoutIsForCurrentPlayer)
{
  const core::Uuid playerDbId;
  ...
  EXPECT_CALL(*mockGame, reset()).Times(1);

  auto queue = std::make_shared<TestUiEventQueue>();
  LogoutMessageConsumer consumer(mockStore, mockGame, queue);
...
TEST(Unit_Bsgalone_Client_Messages_LogoutMessageConsumer,
     DoesNotResetTheGameWhenLogoutIsForAnotherPlayer)
{
  const core::Uuid playerDbId1;
