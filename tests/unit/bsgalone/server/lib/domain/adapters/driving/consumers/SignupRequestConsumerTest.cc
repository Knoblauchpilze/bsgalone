...
namespace {
class MockUseCase : public ForExecutingSignup
{
  public:
  MockUseCase()           = default;
  ~MockUseCase() override = default;

  MOCK_METHOD(void, performSignup, (const SignupData &), (override));
};
} // namespace
     ...
     DelegatesSignupToUseCaseWhenClientIdIsSet)
{
  ...
  EXPECT_CALL(*usecase, performSignup(_))
    .Times(1)
    .WillOnce(Invoke([&captured](const SignupData &data) { captured = data; }));
