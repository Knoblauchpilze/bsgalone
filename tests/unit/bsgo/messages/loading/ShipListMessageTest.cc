...
auto assertMessagesAreEqual(const ShipListMessage &actual, const ShipListMessage &expected)
{
  ...
  for (std::size_t id = 0; id < actualShipsData.size(); ++id)
  {
    ...
    EXPECT_EQ(actualShipData.docked, expectedShipData.docked);
    EXPECT_EQ(actualShipData.jumpTime, expectedShipData.jumpTime);
    EXPECT_EQ(actualShipData.jumpTimeInThreat, expectedShipData.jumpTimeInThreat);
