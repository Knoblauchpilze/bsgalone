...
{
  const SlotComponentMessage expected(Uuid{14}, Uuid{2}, {});
  SlotComponentMessage actual(Uuid{36}, Uuid{1}, utils::Milliseconds(300));
...
{
  const SlotComponentMessage expected(Uuid{14}, Uuid{2}, utils::Milliseconds(250));
  SlotComponentMessage actual(Uuid{36}, Uuid{1}, {});
  actual.setClientId(Uuid{44});
  serializeAndDeserializeMessage(expected, actual);
  assertMessagesAreEqual(actual, expected);
}
...
{
  SlotComponentMessage expected(Uuid{28}, Uuid{67}, utils::Milliseconds(250));
  expected.setClientId(Uuid{119});
  SlotComponentMessage actual(Uuid{51}, Uuid{180}, {});
  serializeAndDeserializeMessage(expected, actual);
  assertMessagesAreEqual(actual, expected);
}
...
{
  const SlotComponentMessage expected(Uuid{28}, Uuid{67}, utils::Milliseconds(250));
