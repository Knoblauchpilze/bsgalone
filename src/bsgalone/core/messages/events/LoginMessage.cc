...
{
  return m_playerDbId.has_value() && m_faction.has_value() && m_role.has_value();
}
...
auto LoginMessage::getFaction() const -> Faction
{
  return m_faction.value();
}
...
void LoginMessage::setFaction(const Faction faction)
{
  m_faction = faction;
}
