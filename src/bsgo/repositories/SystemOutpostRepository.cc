...
constexpr auto UPDATE_SYSTEM_OUTPOST_QUERY_NAME = "system_outpost_update";
...
void SystemOutpostRepository::initialize()
{
  ...
  m_connection->prepare(UPDATE_SYSTEM_OUTPOST_QUERY_NAME, UPDATE_SYSTEM_OUTPOST_QUERY);
}
...
void SystemOutpostRepository::save(const SystemOutpost &outpost)
{
  ...
  if (res.error)
  {
    error("Failed to save system outpost: " + *res.error);
  }
}
