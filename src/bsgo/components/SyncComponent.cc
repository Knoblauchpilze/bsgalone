...
auto generateJitteredSyncInterval() -> chrono::TickDuration
{
  ...
  return chrono::TickDuration::fromInt(jitteredSync);
}
} // namespace
...
void SyncComponent::markAsJustSynced()
{
  markForSync(false);
  m_untilNextSync        = generateJitteredSyncInterval();
  m_elapsedSinceLastSync = chrono::TickDuration();
}

void SyncComponent::update(const chrono::TickData &data)
{
